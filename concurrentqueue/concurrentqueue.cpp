...
size_t moodycamel_cq_try_dequeue_bulk_with_token(MoodycamelCQHandle handle, MoodycamelToken cons_token, MoodycamelValue *values,
                                                 size_t max) {
    ConsumerToken *token = (ConsumerToken *)cons_token;
    return reinterpret_cast<MoodycamelCQPtr>(handle)->try_dequeue_bulk(*token, values, max);
}
