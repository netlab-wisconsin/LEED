...
struct kv_mempool *kv_mempool_create(size_t count, size_t ele_size) {
    ...
    for (size_t i = 0; i < count; i++) kv_mempool_put((struct kv_mempool *)mp, mp->buf + i * ele_size);
...
void kv_mempool_put(struct kv_mempool *_mp, void *ele) {
    ...
    moodycamel_cq_enqueue_with_token(mp->cq, mp->p_tokens[kv_app_get_thread_index()], ele);
}
...
int64_t kv_mempool_get_id(struct kv_mempool *mp, void *ele) { return (uint8_t *)ele - ((struct _kv_mempool *)mp)->buf; }
void *kv_mempool_get_ele(struct kv_mempool *mp, int64_t id) { return ((struct _kv_mempool *)mp)->buf + id; }