...
namespace fawn {
	...
	// bowen
	// empty the rocksdb file
	const char* p = kDBPath;
	remove(p);
	printf("rocksdb delete success");
	...
	//open db
	char *err = NULL;
  	rocksdb_ = rocksdb_open(options, kDBPath, &err);
 	assert(!err);
	//assert(status.ok());
	
	cout << "rocksdb is ok";
	//rocksdb_->Put(rocksdb::WriteOptions(), "key1","value");
	...
	rocksdb_writeoptions_t *writeoptions = rocksdb_writeoptions_create();
  	rocksdb_put(rocksdb_, writeoptions, key, strlen(key), data, strlen(data) + 1, &err);

	//bowen
	//insert to rocksdb
	//rocksdb::Status s = rocksdb_->Put(rocksdb::WriteOptions(), key, data);
	...
	//if(!s.ok()){
	//	fprintf(stderr, "Can't delete data from underlying datastore\n");
        //      return false;
	//}
	...
	rocksdb_readoptions_t *readoptions = rocksdb_readoptions_create();
  	size_t len;
  	char *returned_value =
        rocksdb_get(rocksdb_, readoptions, key, strlen(key), &len, &err);
  	assert(!err);
	data = returned_value;
	//bowen
	//rocksdb read
        //rocksdb::Status s = rocksdb_->Get(rocksdb::ReadOptions(), key, &data);
        //if(!s.ok()){
        //        fprintf(stderr, "Can't delete data from underlying datastore\n");
        //        return false;
        //}
